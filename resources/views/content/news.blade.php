                ...
                @foreach ($news as $oneNew)
                    <div class="col-md-6 col-lg-4">
                        <div class="card h-100">
                            <img class="card-img-top" src="{{ $oneNew->image }}" alt="Card image cap" />
                            <div class="card-body">
                                <h5 class="card-title">{{ $oneNew->title }}</h5>
                                <p class="card-text"
                                    style="max-height: calc(2 * 1.4em);overflow: hidden;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;">
                                    {{ $oneNew->description }}
                                </p>
                                <a href="{{ url('news/' . $oneNew->id) }}" class="btn btn-outline-primary">Подробнее</a>
                            </div>
                        </div>
